...
namespace FormClientes
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (TCClientes.Controls[5] == TCClientes.SelectedTab)
                validarDNI();
        ...
        {
            string dniGuardado = maskedTextBox1.Text;

            if (dniGuardado.Length == 9)
            {
                string numerosDNI = dniGuardado.Substring(0, 8);
                char letraIngresada = dniGuardado[8];
                int i_numerosDNI = int.Parse(numerosDNI);

                char letraCalculada = calcularLetraDNI(i_numerosDNI);


                if (letraCalculada == letraIngresada)
                    MessageBox.Show("DNI correcto!");
                 else
                    MessageBox.Show($"DNI incorrecto. La letra correcta debería ser: {letraCalculada}");
            }
            else
                MessageBox.Show("El DNI debe contener 8 dígitos y 1 letra.");
        }

        private char calcularLetraDNI(int digitosDelDNI)
        {
            string tablaLetras = "TRWAGMYFPDXBNJZSQVHLCKE";
            return tablaLetras[digitosDelDNI % 23];
        }
    }
}
